...
class PluginOptions {
    ...
    public function __construct() {
        ...
        add_filter( 'intermediate_image_sizes_advanced', __CLASS__ .  '::remove_disabled_image_sizes' , 20 , 1 );
    ...
    private static function option_disable_sizes() {
        ?>
            <div class="card">
                ...
                <p><?php esc_html_e( 'Disabling thumbnail sizes stops WP from generating resized images on upload. Only disable if your theme/builder does not use these sizes and you only use PIS images.', 'perfect-image-sizes' ) ?></p>
                <p class="notice notice-info notice-large">
                    <?php esc_html_e( 'You should NOT disable thumbnail size. Select the sizes you want to disable.', 'perfect-image-sizes' ); ?>
                </p>
    ...
    private static function option_delete_cached_images() {
        ?>
            <div class="card">
                <h3><?php esc_html_e( 'Flush cached images', 'perfect-image-sizes' ) ?></h3>
                <p><?php esc_html_e( 'Flush all generated image sizes for all PIS images', 'perfect-image-sizes' ) ?></p>
    ...
    private static function try_update_options() {
        ...
        if ( isset( $_POST['pis_imager_settings_nonce'] ) && wp_verify_nonce( $_POST['pis_imager_settings_nonce'], 'imager_settings' ) ) {
            ...
            if ( isset( $_POST[ 'pis_api_access_path' ])) {
                $access_path = untrailingslashit($_POST[ 'pis_api_access_path' ]);
        ...
        if ( isset( $_POST['pis_nonce'] ) && wp_verify_nonce( $_POST['pis_nonce'], 'delete_all_pis_images' ) ) {
            ...
            echo '<div class="updated"><p>' . esc_html__( 'All cached images have been deleted and will be regenerated on next request.', 'perfect-image-sizes' ) . '</p></div>';
        ...
        if ( isset( $_GET['delete-pis-image'], $_GET['ids'], $_GET['pis_nonce'] ) && wp_verify_nonce( $_GET['pis_nonce'], 'delete_pis_image' ) ){
            ...
            if( ! empty( $ids ) ){
                ...
                echo '<div class="updated"><p>' . esc_html__( 'All cached images for this media file have been deleted and will be regenerated on next request.', 'perfect-image-sizes' ) . '</p></div>';
            }
        }
